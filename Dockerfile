...
ARG MOODLE_URL="https://download.moodle.org/download.php/direct/stable501/moodle-latest-501.tgz"  # <— NEW
...
# Optional: raise PHP upload limits (uncomment if you want it baked in)
RUN printf "%s\n" \
  ...
  "max_input_time     = 600" \
  "memory_limit       = 1024M" \
  > /opt/bitnami/php/etc/conf.d/99-upload.ini

COPY rootfs /
# remove this run as need - 3 lines
RUN set -eux; \         
    find /opt/bitnami/scripts -type f -name '*.sh' -exec sed -i 's/\r$//' {} +; \
    find /opt/bitnami/scripts -type f -name '*.sh' -exec chmod +x {} +
